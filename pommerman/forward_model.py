    ...
    def run(self,
        ...
        Args:
          ...
          is_partially_observable: Whether the board is partially observable or
            not. Only applies to TeamRadio. 游戏局面board是否部分可见。只适用于TeamRadio。
          agent_view_size: If it's partially observable, then the size of the
            square that the agent can view. 如果它是部分可见的，那么代理可以看到的正方形的大小。
          action_space: The actions that each agent can take. 每个agent可以操作的行动
