...
def position_is_passable(board, position, enemies):
    ...
    return all([
        any([
            position_is_agent(board, position),  #判断是否有智能体
            position_is_powerup(board, position),#增强的道具
            position_is_passage(board, position) #判断是否是通道
        ]), not position_is_enemy(board, position, enemies) #判断是不是对手，敌人
    ])
