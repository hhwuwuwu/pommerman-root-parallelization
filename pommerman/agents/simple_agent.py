...
class SimpleAgent(BaseAgent):
    ...
    def act(self, obs, action_space):
        def convert_bombs(bomb_map): #个人感觉是把炸弹范围转换位矩阵
        ...
        #采用迪杰斯特拉算法计算出当前位置到地图上任何一点的最短距离，存放在dist数组中
        #统计地图上任何一个标记的点对应的含义，存放在items数组中
        #统计自己的位置采用迪杰斯特拉算法到达地图上任何一个点的前一个位置在哪里。存放在pre数组中。

        # Move if we are in an unsafe place.当我们的位置比较危险的时候，移动到一个比较安全的位置
        ...
        # Lay pomme if we are adjacent to an enemy.当我们接近敌人的时候，我们放一个炸弹
        ...
        #如果在两步可到达的位置内有一个好项目（即三个额外奖励），则移向它
        ...
        #如果在两步可到达的范围内有一个木墙，并且你有一个炸弹，那么向木墙移动
    ...
    def _djikstra(board, my_position, bombs, enemies, depth=None, exclude=None):
        ...
        def out_of_range(p_1, p_2):
            '''Determines if two points are out of rang of each other确定两个点是否不在彼此的范围内'''
            x_1, y_1 = p_1
            x_2, y_2 = p_2
        ...
        for r in range(max(0, my_x - depth), min(len(board), my_x + depth)):
            for c in range(max(0, my_y - depth), min(len(board), my_y + depth)):
                ...
                items[item].append(position) #探究棋局上每个位置对应的标志
        ...
        for bomb in bombs: #判断当前位置是不是在炸弹波及的范围里面
    ...
    def _directions_in_range_of_bomb(self, board, my_position, bombs, dist):
        ...
        for bomb in bombs:
            ...
            if distance > bomb_range:
                continue
            #如果最短距离在爆炸范围之内，则会继续向下运行，否则跳出本次循环

            #如果我的位置就是炸弹位置，那么所有的方向都是危险的
